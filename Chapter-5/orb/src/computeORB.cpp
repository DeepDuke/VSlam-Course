...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        int u0 = kp.pt.x, v0 = kp.pt.y;
        // Filter boundary points
        if(u0 < half_patch_size || v0 < half_patch_size || (u0+half_patch_size) > image.cols || (v0+half_patch_size) > image.rows )
            continue;
        // Compute coordinates
        double m10 = 0.0, m01 = 0.0;
        for(int v = v0 - half_patch_size; v < v0 + half_patch_size; v++){
            for(int u = u0 - half_patch_size; u < u0 + half_patch_size; u++){
                m01 += v*image.at<uchar>(v, u); // (col, row)
                m10 += u*image.at<uchar>(v, u);
            }
        }
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            double sin_theta = sin(kp.angle / 180.0 * pi);
            double cos_theta = cos(kp.angle / 180.0 * pi);

            cv::Point2f p(ORB_pattern[4*i], ORB_pattern[4*i+1]);
            cv::Point2f q(ORB_pattern[4*i+2], ORB_pattern[4*i+3]);

            cv::Point2f pr(cos_theta*p.x - sin_theta*p.y, sin_theta*p.x + cos_theta*p.y);
            cv::Point2f qr(cos_theta*q.x - sin_theta*q.y, sin_theta*q.x + cos_theta*q.y);

            cv::Point2f  pp(kp.pt + pr);
            cv::Point2f qq(kp.pt + qr);
            ...
            d[i] = image.at<uchar>(pp.y, pp.x) > image.at<uchar>(qq.y, qq.x) ? 0:1;
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i = 0; i < desc1.size(); i++){
        ...
        for(int j = 0; j < desc2.size(); j++){
            ...
            int distance = 0;
            for(int k = 0; k < 256; k++){
                distance += desc1[i][k] == desc2[j][k]?0:1;
            }
    ...
    // END YOUR CODE HERE
    double end_t = (double)cv::getTickCount();
    double delta_t = (end_t - start_t) / cv::getTickFrequency();
    cout << "brute-force matching time is: " << delta_t << endl;
//    for (auto &m: matches) {
