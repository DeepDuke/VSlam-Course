...
string file_1 = "../src/left.png";  // first image
string file_2 = "../src/right.png";  // second image
string file_3 = "../src/disparity.png";
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < kp2_single.size(); i++) {
        if (success_single[i]) {
            ...
            cv::line(img2_single, kp1[i].pt, kp2_single[i].pt, cv::Scalar(250, 0, 0));
    ...
    cout << "Single Layer Optical Flow:" <<"\n has ：" << count << "\n [0, 5]:" << count1 << "\n (5,10]:" << count2 << "\n (10,20]:" << count3 << "\n [20,++):" << count4 << endl;
    ...
    cout << "Four-Layer Pyramid Optical Flow:" << "\n has:" << count << "\n [0, 5]:" << count1 << "\n (5,10]: " << count2 << "\n (10,20]: " << count3 << "\n [20,++): " << count4 << endl;
    ...
    for (int i = 0; i < pt2.size(); i++) {
        if (status[i]) {
            ...
            cv::line(img2_CV, pt1[i], pt2[i], cv::Scalar(250, 0, 0));
    ...
    cout << "OpenCv's Optical Flow:" <<"\n has:" << count << "\n [0, 5]: " << count1 << "\n (5,10]: " << count2 << "\n (10,20]: " << count3 << "\n [20,++): " << count4 << endl;

    cv::imshow("tracked single level", img2_single);
    cv::imshow("tracked multi level", img2_multi);
    cv::imshow("tracked by opencv", img2_CV);
    cv::waitKey(0);
    cv::imwrite("../src/multi_cal.jpg",img2_multi);
    cv::imwrite("../src/single_cal.jpg",img2_single);
    cv::imwrite("../src/opencv_cal.jpg",img2_CV);
    return 0;
}
...
void OpticalFlowSingleLevel(
    ...
    for (size_t i = 0; i < kp1.size(); i++) {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            // compute cost and jacobian both x and y are [-4,3]
